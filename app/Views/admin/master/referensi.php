                ...
                <ul class="nav nav-tabs" id="referensiTabs" role="tablist">
                    <?php foreach ($tabs as $key => $tab): ?>
                        <li class="nav-item" role="presentation">
                            <a class="nav-link <?= $key === $viewState['activeTab'] ? 'active' : '' ?>" id="tab-<?= esc($key) ?>-link" data-bs-toggle="tab" href="#tab-<?= esc($key) ?>" role="tab">
            ...
            <div class="card-body">
                <div class="tab-content" id="referensiTabsContent">
                    <?php foreach ($tabs as $key => $tab): ?>
                        ...
                        ?>
                        <div class="tab-pane fade <?= $key === $viewState['activeTab'] ? 'show active' : '' ?>" id="tab-<?= esc($key) ?>" role="tabpanel">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h6 class="mb-0 text-muted">
                                    <i class="<?= esc($tab['icon']) ?> me-1"></i>
                                    Daftar <?= esc($tab['label']) ?>
                                    &mdash; <strong><?= esc((string) count($tab['items'])) ?></strong> data
                                </h6>
                                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modal-tambah-<?= esc($key) ?>">
                                    <i class="bi bi-plus-lg me-1"></i>Tambah <?= esc($tab['label']) ?>
                                </button>
                            </div>
                            ...
                            <?php if (empty($tab['items'])): ?>
                                <div class="dosen-empty-state">
                                    ...
                                    <p class="mb-1">Belum ada data <?= esc($tab['label']) ?></p>
                                                        ...
                                                        <td class="text-center btn-action-group admin-action-group">
                                                            ...
                                                            <?php else: ?>
                                                                <button type="button" class="btn btn-warning btn-sm"
                                                                    ...
                                                                    data-admin-modal-title-text="<i class='bi bi-pencil-square me-2'></i>Edit <?= esc($tab['label']) ?>"
                            ...
                            <div class="modal fade" id="modal-tambah-<?= esc($key) ?>" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form action="<?= site_url('admin/master/referensi/store/' . $key) ?>" method="post" data-submit-state-form>
                                            <?= csrf_field() ?>
                                            <div class="modal-header">
                                                <h5 class="modal-title"><i class="<?= esc($tab['icon']) ?> me-2"></i>Tambah <?= esc($tab['label']) ?></h5>
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <?php endif; ?>
                                                <div class="mb-3">
                                                    <label class="form-label fw-semibold"><?= esc($tab['fieldLabel']) ?> <span class="text-danger">*</span></label>
